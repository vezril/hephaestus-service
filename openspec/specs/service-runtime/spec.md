...
Define the minimal runnable Hephaestus service: a Pekko typed `ActorSystem` exposing a `/health` endpoint, graceful startup/shutdown, env-overridable HOCON configuration, a startup-verified media toolchain (ffmpeg/libvips), and a Docker image that bundles those tools.
