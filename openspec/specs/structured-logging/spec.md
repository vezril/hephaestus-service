...
Define how Hephaestus emits logs so they are machine-readable across the constellation: single-line
JSON in the container image (for Loki + error tracking + the self-healing feedback loop) and
human-readable text in local development, using the constellation-wide field schema so queries
resolve identically across services.
...
`logstash-logback-encoder`, and the `service` field SHALL be `hephaestus`.
