...
Define the opt-in end-to-end verification of the composed worker against the real ApolloStorage and
HermesMQ services (their published images via testcontainers): a `ProcessMediaJob` published to a
real `media.ingest` subscription, referencing a real original in Apollo, results in the derivative
set written content-addressed to Apollo and a `MediaProcessed` on `media.processed` — the true
"does the whole loop close" check that the per-milestone stub tests can't give.
