# apollo-io Specification

## Purpose

Define how Hephaestus reads originals from and writes derivatives to Apollo's object store over
gRPC: a client generated from the single-source Lexicon contract, streaming reads with end-to-end
md5 verification, atomic content-addressed streaming writes, a head/exists check, and a typed
...
streamed bytes and verify it equals the metadata header's md5. A mismatch, a missing header md5, or
a truncated stream SHALL be a **terminal** (non-retriable) failure.
...
#### Scenario: Edge case — missing header md5 is terminal
- **Given** a `GetObject` header whose md5 is empty (an Apollo contract violation for an original)
- **When** the client reads it
- **Then** the read fails terminally rather than passing unverified bytes through as success
...
#### Scenario: Edge case — a transient fault mid-stream is retriable
- **Given** the payload stream fails partway (Apollo drop / `DEADLINE_EXCEEDED`)
- **When** the failure reaches the consumer
- **Then** it is a typed error flagged `retriable = true` (not a raw status exception)
