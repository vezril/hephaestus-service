...
Define the buildable sbt/Scala skeleton for Hephaestus: a strict two-module build (pure `core`, Pekko `server`), git-tag-derived semantic versioning, a protected two-branch strategy, and CI that verifies formatting, compilation, and tests on every pull request.
