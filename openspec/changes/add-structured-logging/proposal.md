...
- **structured-logging** (new): `hephaestus` emits one JSON object per log event when containerized
...
- `logback.xml`: a `json` appender (`LogstashEncoder`, `<customFields>{"service":"hephaestus"}</customFields>`)
