...
- **When** it is decoded
- **Then** it yields a media descriptor the §2 pipeline can run (source bucket/object, type, want)
...
that `add-result-reporting` fills; this capability ships a no-op/logging default so the ack
invariant holds and is testable now.
