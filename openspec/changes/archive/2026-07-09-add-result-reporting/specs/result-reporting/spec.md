...
map each terminal `MediaError` case to a stable `JobError.code`.
...
The publisher SHALL be invoked before the job is acknowledged (the §3 invariant), and re-publishing
an equivalent result on redelivery SHALL be safe — a `MediaProcessed` for a given `jobId` is
byte-identical across redeliveries and Artemis applies it at most once.
