...
`variant`/`codec`), and the stamped `derivativeSpecVersion` — the value §4 publishes as
`MediaProcessed`. On corrupt/unsupported input it SHALL yield a **terminal** failure instead. (This
change produces the value in-process; publishing it over HermesMQ is `add-result-reporting`.)
